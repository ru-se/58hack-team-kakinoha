...
export default function QuizPage() {
  ...
  const currentReviewQ = REVIEW_QUESTIONS[reviewIndex];
  const isCorrect = selectedAnswer === currentReviewQ?.correctIndex;
  ...
  const handleNextReview = () => {
    if (reviewIndex + 1 >= REVIEW_QUESTIONS.length) {
      setPhase('completed');
        ...
        <header className="flex items-center justify-between border-b-[6px] border-black bg-[#2d5be3] px-4 py-3 text-white">
          ...
          {(phase === 'review') && (
            <span className="text-sm font-black text-white/80">
              {reviewIndex + 1} / {REVIEW_QUESTIONS.length}
            </span>
          )}
        </header>
            ...
            <div className="flex flex-col gap-5 w-full">
              ...
              <div className="flex flex-col gap-3">
                {currentReviewQ.options.map((opt, idx) => {
                  ...
                  const isRight = idx === currentReviewQ.correctIndex;
                  let bg = 'bg-white';
                  if (isAnswered) {
              ...
              {isAnswered && (
                <div className="flex flex-col gap-3 animate-[fadeInUp_0.3s_ease-out]">
                  ...
                  >
                    {isCorrect ? currentReviewQ.explanation : `❌ 不正解...\n正解は「${currentReviewQ.options[currentReviewQ.correctIndex]}」です。\n\n${currentReviewQ.explanation.replace('✅ 正解！', '')}`}
                  </div>
                  <button
                  ...
                  >
                    {reviewIndex + 1 >= REVIEW_QUESTIONS.length ? '結果を見る 🎉' : '次の問題へ →'}
                  </button>
                </div>
              )}
            </div>
